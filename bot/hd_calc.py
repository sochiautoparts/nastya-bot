"""Human Design (Bodygraph) Calculation Engine v4.0
====================================================
...
v4.0 ИСПРАВЛЕНИЯ:
  - КРИТИЧЕСКОЕ: Тип определяется через BFS-путь мотор→горло, а не только прямые каналы!
    Это исправляет ошибку, когда Манифестирующие Генераторы определялись как Генераторы
    из-за косвенных путей (например: Крестец→G-центр→Горло через каналы 29-46 + 10-20)
...
def _is_motor_connected_to_throat(
    ...
    """Проверить, есть ли путь от любого моторного центра к Горловому через цепочку каналов.

    В ДЧ мотор→горло путь может быть ПРЯМЫМ (один канал) или ЧЕРЕЗ ДРУГИЕ ЦЕНТРЫ
    (цепочка каналов). Например:
      - Крестец → (канал 29-46) → G-центр → (канал 10-20) → Горло = МГ
      - Корень → (канал 53-42) → Крестец → (канал 34-20) → Горло = МГ
      - Сердце → (канал 21-45) → Горло = МГ (прямой)

    Использует BFS по графу центров, связанных полными каналами.
    """
    if 'throat' not in defined_centers:
        return False

    # Строим граф: центр → множество соседних центров через полные каналы
    ...
    # BFS от горла — ищем, достижим ли хоть один мотор
    visited: Set[str] = set()
    queue = ['throat']
    while queue:
        current = queue.pop(0)
        if current in visited:
            continue
        visited.add(current)
        # Если текущий центр — мотор (кроме горла), путь найден
...
def _determine_type(
    ...
    Правила (по Ра Уру Ху):
      ...
      - Манифестирующий Генератор: Сакральный определён + мотор соединён с Горлом
        через цепочку полных каналов (прямой или косвенный путь).
        Моторы: Сакральный, Корневой, Солнечное Сплетение, Сердце/Воля
      - Генератор: Сакральный определён, НО нет мотор→горло пути
      - Манифестор: мотор→горло путь есть, НО Сакральный НЕ определён
      - Проектор: всё остальное

    v4.0 ИСПРАВЛЕНИЕ: проверяем ЦЕПОЧКИ каналов (BFS), а не только прямые каналы.
    Это критически важно — предыдущая версия пропускала косвенные пути
    (например Крестец→G-центр→Горло), что давало неправильный тип «Генератор»
    вместо «Манифестирующий Генератор».
    """
    sacral_defined = 'sacral' in defined_centers

    # Проверяем связь Горла с мотором через цепочку каналов (BFS)
    throat_connected_to_motor = _is_motor_connected_to_throat(
        defined_centers, complete_channels)
